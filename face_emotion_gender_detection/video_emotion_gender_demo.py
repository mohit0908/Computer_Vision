...
input_filepath = 'Video'

def inference(file):
    input_filename = file
    output_path = os.path.join(input_filepath, input_filename.split('.')[0]+'_output.mp4')

    counter = 0
    frame_process_counter = 0
    # starting video streaming
    cv2.namedWindow('window_frame')
    input_video = os.path.join(input_filepath, input_filename)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--file', required = 'True', help = 'name of input file')
    args = vars(ap.parse_args())
    inference(args['file'])
    print(args['file'])

# Usage: python video_emotion_gender_demo.py filename